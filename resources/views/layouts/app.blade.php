...
<html lang="{{ app()->getLocale() }}">
    ...
    <body>
        ...
        <script>           
            ...
            var config = {
                rutes: [
                    { inici: "{{ URL::to('') }}", ajax_subestacio: "{{ URL::to('ajax_subestacio') }}", ajax_buscador: "{{ URL::to('ajax_buscador') }}", editarPerfil: "{{ URL::to('editarPerfil') }}", ajax_eliminar: "{{ URL::to('ajax_eliminar') }}", ajax_eliminarCombinacio: "{{ URL::to('ajax_eliminarCombinacio') }}" @if (Route::has('login')) @auth , id_usuari: "{{ Auth::user()->id }}" @endauth @endif}
                ]
            };
        </script>
